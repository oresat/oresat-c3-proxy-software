...
_CONFIG_CONST_BITS = 0x4000  # must be 1 per datasheet
...
_VBUSCT_140US  = 0x0000
_VBUSCT_204US  = 0x0040
_VBUSCT_332US  = 0x0080
_VBUSCT_588US  = 0x00C0
_VBUSCT_1100US = 0x0100
_VBUSCT_2116US = 0x0140
_VBUSCT_4156US = 0x0180
_VBUSCT_8244US = 0x01C0
...
_VSHCT_332US  = 0x0010
_VSHCT_588US  = 0x0018
_VSHCT_1100US = 0x0020
_VSHCT_2116US = 0x0028
_VSHCT_4156US = 0x0030
_VSHCT_8244US = 0x0038

# Operating mode (MODE[2:0] bits at 2:0)
_MODE_POWERDOWN              = 0x0000
_MODE_SHUNT_TRIGGERED        = 0x0001
_MODE_BUS_TRIGGERED          = 0x0002
_MODE_SHUNT_BUS_TRIGGERED    = 0x0003
_MODE_ADC_OFF                = 0x0004
_MODE_SHUNT_CONTINUOUS       = 0x0005
_MODE_BUS_CONTINUOUS         = 0x0006
_MODE_SHUNT_BUS_CONTINUOUS   = 0x0007
...
class INA226:
    ...
    def __init__(self, i2c: SMBus, addr=_INA226_ADDR):
        self.i2c = i2c
        self.addr = addr
        ...
        self.calibrate(r_shunt_ohms=0.01, max_expected_amps=3)

    # ----------------------------
    # Low-level I2C register access
    # ----------------------------
    def _write_u16(self, reg, value):
        toWrite = ((value & 0xFF) << 8) | ((value >> 8) & 0xFF)

        #self.i2c.writeto_mem(self.addr, reg, self._buf)

        #data = bytearray([reg]) + (self._buf)
        #print("buf parts: ", reg, self._buf)
        #print("(lib)ina226 i2c write ", self.addr, data)
        #write = i2c_msg.write(self.addr, data)
        #self.i2c.write_block_data(self.addr, reg, self._buf)
        #self.i2c.i2c_rdwr(write)
        self.i2c.write_word_data(self.addr, reg, toWrite)
